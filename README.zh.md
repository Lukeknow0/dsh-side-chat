...
- **关闭立即收口：** 先让客户端所有晚到回调失效，再取消创建或生成、释放实时子会话，并尝试归档清理。
...
3. 输入一个聚焦的临时问题。侧边智能体可以读取继承的上下文和只读资源。
4. 点击关闭、按 Escape、再次使用快捷键或标题栏按钮，即可结束。

切换到另一个父会话时，当前 Side Chat 会自动关闭。每个父会话同一时间只允许一个 Side Chat。
...
**在 DSH 0.1.0-rc.7 中，“临时”不等于保证物理擦除。** 该版本没有公开的持久会话日志删除 API。关闭 Side Chat 时，本插件会：
...
这是一个 MVP，公开 API 可能在 1.0 前调整。以下不变量不会轻易改变：安全的已完成回合分叉、禁止写入、不修改父会话、一个抽屉只有一个所有者、关闭时主动清理。
